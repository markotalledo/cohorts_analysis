CREATE OR REPLACE VIEW SNP_SANDBOX.COHORTS_MONTHLY_CUSTOMERS AS
(
with leader_tier as
...
(select distinct
 ...
 leader_tier,
 customer_tier,
 date_trunc('month',leader_activation_date) leader_activation_month,
 ...
 datediff('month',customer_activation_month,date_trunc('month',create_date_time_tz::date)) relative_month,
sum(case when coupon like any ('FAV-%','POC-FAV-%','MAY-%','POC-MAY-%') then net_value else gross_value end)
 over (partition by b.dynamo_customer_id,t.store_url_adj,leader_tier,customer_tier,leader_activation_month,cluster,customer_activation_month,calendar_month,relative_month) as
 gmv
,count(distinct order_number)
    over (partition by b.dynamo_customer_id,t.store_url_adj,leader_tier,customer_tier,leader_activation_month,cluster,customer_activation_month,calendar_month,relative_month) as orders,
sum(quantity_fixed)
    over (partition by b.dynamo_customer_id,t.store_url_adj,leader_tier,customer_tier,leader_activation_month,cluster,customer_activation_month,calendar_month,relative_month) as units,
sum(case when create_date_time_tz::date between customer_activation_date and dateadd('day',30,customer_activation_date)
     then (case when coupon like any ('FAV-%','POC-FAV-%','MAY-%','POC-MAY-%') then net_value else gross_value end)
     else 0 end
    ) over (partition by b.dynamo_customer_id) as ticket_1m,
 case when ticket_1m<100 then 'a.Menos de 100 soles' else 'b.Mas de 100 soles' end as tag_ticket_1m
...
select
customer_activation_month,
leader_activation_month,
relative_month,
cluster,
leader_tier,
customer_tier,
...
sum(units) as units,
ROUND(avg(ticket_1m),2) as avg_ticket_1m
...
select * from SNP_SANDBOX.COHORTS_MONTHLY_CUSTOMERS  where RELATIVE_MONTH < 0;