CREATE OR REPLACE VIEW SNP_SANDBOX.COHORTS_MONTHLY_LEADERS AS
(
with leader_tier as
...
cohort_pre as
(select
 case when b.store_url='gruporomost' then 'gruporomo'
 when m.leader_class='mayorista_test_st' then storeurl_origin
 else b.store_url end as store_url_adj,
 t.tier,
 case when t.tier='mayorista' then 'mayorista' else 'no mayorista' end as cluster,
 date_trunc('month',t.activation_date) as activation_month,
 date_trunc('month',create_date_time_tz::date) calendar_month,
 datediff('month',activation_month,calendar_month) relative_month,
 sum(case when coupon like any ('FAV-%','POC-FAV-%','MAY-%','POC-MAY-%') then net_value else gross_value end) gmv,
 count(distinct order_number) orders,
 sum(quantity_fixed) units,
 count(distinct b.dynamo_customer_id) customers,
 count(distinct case when ct.customer_tier<>'leader' and date_trunc('month',create_date_time_tz::date) = date_trunc('month',ct.customer_activation_date)
 then b.dynamo_customer_id else null end) as new_customers,
  count(distinct case when ct.customer_tier<>'leader' and date_trunc('month',create_date_time_tz::date) <> date_trunc('month',ct.customer_activation_date)
 then b.dynamo_customer_id else null end) as old_customers,
   count(distinct case when ct.customer_tier='leader'
 then b.dynamo_customer_id else null end) as leader_customers
 ...
 from
 journey.base) b
 left join FAVODATA.SNP_UNTRUSTED.MAYORISTAS m on m.dynamo_leader_id=b.dynamo_leader_id and m.leader_class<>'top_mayorista'
...
and relative_month > 0
 group by 1,2,3,4,5,6
),
rank as (
select store_url_adj,
    activation_month,
    relative_month,
    gmv,
    cluster,
   sum(gmv) over (partition by activation_month,relative_month,cluster order by gmv desc rows between unbounded preceding and current row) as cum_gmv, --POR CLUSTER: MAYORISTA/NO MAYORISTA
   sum(gmv) over (partition by activation_month,relative_month order by gmv desc rows between unbounded preceding and current row) as cum_gmv_total, -- DE TODO FAVO
    sum(gmv) over (partition by activation_month,relative_month,cluster) as gmv_total, --POR CLUSTER
     sum(gmv) over (partition by activation_month,relative_month) as gmv_total_conso, -- DE TODO FAVO
...
r.percentile,
r.percentile_total,
 case when r.percentile is null then 'churn_on_2nd_month'
 when r.percentile<=0.8 then 'top_80%_revenue' else 'bottom_20%_revenue' end as pareto,
 case when r.percentile_total is null then 'churn_on_2nd_month'
 when r.percentile_total<=0.8 then 'top_80%_revenue' else 'bottom_20%_revenue' end as pareto_total
 from cohort_pre c
 left join rank r on r.store_url_adj=c.store_url_adj and r.activation_month=c.activation_month and r.relative_month=1
)
select
activation_month,
relative_month,
...
SELECT * FROM SNP_SANDBOX.COHORTS_MONTHLY_LEADERS
